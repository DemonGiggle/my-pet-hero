...
function migrateSaveData(raw: unknown): { migrated: PetState; fromVersion: number; changed: boolean } {
  ...
  let changed = false;
  const migrated = cloneJson(raw) as Record<string, unknown>;

  while ((migrated.version as number) < CURRENT_SAVE_VERSION) {
    const currentVersion = migrated.version as number;

    if (currentVersion === 2) {
      const species = migrated.species as Species;
    ...
    if (currentVersion === 4) {
      ...
      hero.adventureLog = adventureLog.map(entry => {
        ...
        if (!combat) return entry;

        const normalizeCombatant = (combatant: unknown) => {
          if (!isRecord(combatant)) return combatant;
          return {
            ...combatant,
...
async function backupOriginalSave(id: string, raw: string, fromVersion: number, dataDir: string): Promise<void> {
...
function resolveDefaultDataDir(): string {
  ...
  if (xdgStateHome) return path.resolve(xdgStateHome, 'my-pet-hero', 'pets');
...
async function migrateLegacyPets(dataDir: string): Promise<void> {
  if (dataDir === LEGACY_DATA_DIR) return;
  if (!(await pathExists(LEGACY_DATA_DIR))) return;
...
export async function ensureDataDir(dataDir = DEFAULT_DATA_DIR): Promise<string> {
