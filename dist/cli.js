...
function formatAdventureReport(result) {
    ...
    if (result.pet.hero.dungeon.currentExpedition) {
        ...
        lines.push(...formatExpeditionSummary(result.pet.hero.dungeon.currentExpedition).map(line => `  ${line}`));
    }
    ...
    for (const item of adventures) {
        ...
        lines.push(`  ${item.text}`);
        if (item.roomSummary)
            lines.push(`  房型：${item.roomSummary}`);
        if (item.rewards && item.rewards.length > 0)
            lines.push(`  收益：${item.rewards.join('、')}`);
        if (item.combat) {
...
async function dungeonPreview(id) {
    const pet = await loadPet(id);
...
async function combatPreview(id) {
    const pet = await loadPet(id);
    const result = simulatePet(pet);
...
async function main() {
    ...
    if (cmd === 'combat-preview') {
        const id = getArg('id');
        if (!id)
            throw new Error('combat-preview 需要 --id');
    ...
    const id = getArg('id');
    if (!id)
        throw new Error(`${cmd} 需要 --id`);
    if (cmd === 'status')
        return printStatus(id);
    if (cmd === 'feed' || cmd === 'play' || cmd === 'clean')
        return mutate(id, cmd);
