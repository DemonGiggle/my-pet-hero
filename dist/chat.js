...
export function parseChatCommand(input) {
    ...
    if (command === 'help')
        return { namespace: 'pet', action: 'help', raw: input, tokens };
    if (command === 'status')
        return { namespace: 'pet', action: 'status', heroId: firstArg, raw: input, tokens };
    if (command === 'report')
        return { namespace: 'pet', action: 'report', heroId: firstArg, raw: input, tokens };
...
function chatStatePath(dataDir = DEFAULT_DATA_DIR) {
    return path.join(path.dirname(dataDir), CHAT_STATE_FILE);
}
export async function loadChatPreferences(dataDir = DEFAULT_DATA_DIR) {
...
export async function saveChatPreferences(preferences, dataDir = DEFAULT_DATA_DIR) {
    const resolvedDataDir = await ensureDataDir(dataDir);
    const filePath = chatStatePath(resolvedDataDir);
