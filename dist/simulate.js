...
export function simulatePet(pet, nowIso = new Date().toISOString()) {
    ...
    pet.needs.mood = clamp(pet.needs.mood
    ...
    pet.needs.health = clamp(pet.needs.health
    ...
    for (const bucket of bucketHours(pet.lastSimulatedAt, nowIso, 2)) {
        ...
        if (unit < 0.15) {
            ...
            const event = { at: bucket, type: 'messy', delta: { hygiene: -8 }, text: pickOne([...EVENT_COPY.messy], (unit - 0.15) / 0.12) };
            pet.needs.hygiene = clamp(pet.needs.hygiene - 8);
            events.push(event);
        }
        else if (unit > 0.92) {
            const event = { at: bucket, type: 'proud', delta: { mood: 5 }, text: pickOne([...EVENT_COPY.proud], (unit - 0.92) / 0.08) };
